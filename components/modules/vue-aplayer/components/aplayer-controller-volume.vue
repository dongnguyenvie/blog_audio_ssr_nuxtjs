...
export default class AplayerControllerVolume extends Vue {
  @Prop({ type: Number }) volume!: number
  @Prop({ type: Boolean }) muted!: boolean
  @Prop({ type: String }) theme!: string

  @Ref('bar') readonly bar!: HTMLDivElement
  ...
  adjustVolume(e: MouseEvent) {
    let percentage = (barHeight - e.clientY + getElementViewTop(this.bar)) / barHeight
